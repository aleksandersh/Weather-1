...
public interface WeatherDao {
     ...
     * @deprecated От данного метода решил отказаться, так как вряд ли пользователи будут
     * так часто менять языки и изменять данные из разных источников, но исходники пусть останутся.
     */
    @Deprecated
    Observable<List<City>> getFavoriteCitiesObservable(String lang);

    /**
     * Получение {@link Single<City>} источника избранных городов для заданного языка локализации.
     * Приоритет выбора локализации данных о городе:
     * 1) Код языка, переданный в параметре.
     * 2) Язык по-умолчанию.
     * 3) Любая другая локализация, присутствующая в базе данных.
     * <p>
